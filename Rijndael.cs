...
namespace RijndaelMedium
{
    public class Rijndael : IRijndael
    {
        ...
        // Implementation based on the FastAES of Bouncycastle
        private byte[] Key;
        ...
        public byte[] Encrypt(byte[] data)
        {
            ...
            // unsafe is here to grant direct access to the application its memory
            unsafe 
            {
                fixed (byte* pBlock = data)
                {
                    int curRound = 0;
        ...
        public byte[] Decrypt(byte[] data)
        {
            ...
            unsafe
            {
                fixed (byte* pBlock = data)
                {
                    ...
                    {
                        wk = WorkingKey[--curRound];
                        r0 = InvTable0[C0 & 255] ^ InvTable1[(C3 >> 8) & 255] ^ InvTable2[(C2 >> 16) & 255] ^ InvTable3[C1 >> 24] ^ wk[0];
                        r1 = InvTable0[C1 & 255] ^ InvTable1[(C0 >> 8) & 255] ^ InvTable2[(C3 >> 16) & 255] ^ InvTable3[C2 >> 24] ^ wk[1];
                        r2 = InvTable0[C2 & 255] ^ InvTable1[(C1 >> 8) & 255] ^ InvTable2[(C0 >> 16) & 255] ^ InvTable3[C3 >> 24] ^ wk[2];
                        r3 = InvTable0[C3 & 255] ^ InvTable1[(C2 >> 8) & 255] ^ InvTable2[(C1 >> 16) & 255] ^ InvTable3[C0 >> 24] ^ wk[3];
                        wk = WorkingKey[--curRound];
                        C0 = InvTable0[r0 & 255] ^ InvTable1[(r3 >> 8) & 255] ^ InvTable2[(r2 >> 16) & 255] ^ InvTable3[r1 >> 24] ^ wk[0];
                        C1 = InvTable0[r1 & 255] ^ InvTable1[(r0 >> 8) & 255] ^ InvTable2[(r3 >> 16) & 255] ^ InvTable3[r2 >> 24] ^ wk[1];
                        C2 = InvTable0[r2 & 255] ^ InvTable1[(r1 >> 8) & 255] ^ InvTable2[(r0 >> 16) & 255] ^ InvTable3[r3 >> 24] ^ wk[2];
                        C3 = InvTable0[r3 & 255] ^ InvTable1[(r2 >> 8) & 255] ^ InvTable2[(r1 >> 16) & 255] ^ InvTable3[r0 >> 24] ^ wk[3];
                    }
                    ...
                    //Final round of SubBytes, ShiftRows and AddRoundKey
                    wk = WorkingKey[--curRound];
                    C0 = (uint)InvSbox[r0 & 255] ^ (((uint)InvSbox[(r3 >> 8) & 255]) << 8) ^ (((uint)InvSbox[(r2 >> 16) & 255]) << 16) ^ (((uint)InvSbox[r1 >> 24]) << 24) ^ wk[0];
                    C1 = (uint)InvSbox[r1 & 255] ^ (((uint)InvSbox[(r0 >> 8) & 255]) << 8) ^ (((uint)InvSbox[(r3 >> 16) & 255]) << 16) ^ (((uint)InvSbox[r2 >> 24]) << 24) ^ wk[1];
                    C2 = (uint)InvSbox[r2 & 255] ^ (((uint)InvSbox[(r1 >> 8) & 255]) << 8) ^ (((uint)InvSbox[(r0 >> 16) & 255]) << 16) ^ (((uint)InvSbox[r3 >> 24]) << 24) ^ wk[2];
                    C3 = (uint)InvSbox[r3 & 255] ^ (((uint)InvSbox[(r2 >> 8) & 255]) << 8) ^ (((uint)InvSbox[(r1 >> 16) & 255]) << 16) ^ (((uint)InvSbox[r0 >> 24]) << 24) ^ wk[3];
        ...
        //BEGIN_CHALLENGE
        #region Helpers
        private void ExpandKey(bool bEncrypt)
        {
            int keyLength = Key.Length / 4;  // key length in words
            Rounds = keyLength + 6;

            WorkingKey = new uint[Rounds + 1][]; // 4 words in a block
            ...
            {
                WorkingKey[t >> 2][t & 3] = LeToUInt32(Key, i);
                i += 4;
            }
            ...
            {
                uint temp = WorkingKey[(i - 1) >> 2][(i - 1) & 3];
                if ((i % keyLength) == 0)
                {
