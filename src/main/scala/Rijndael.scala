class Rijndael extends IRijndael {
  ...
  override def encrypt(byte: Array[Byte], key: Array[Byte]): Array[Byte] = {
    //BEGIN_CHALLENGE
    val workingKey = expandKey(key, true)
    val buffer: Array[Int] = byte.grouped(4).map(setHolder).toArray
    encipher(buffer, workingKey, (key.length / 4) + 6)
  ...
  override def decrypt(byte: Array[Byte], key: Array[Byte]): Array[Byte] = {
    //BEGIN_CHALLENGE
    val workingKey = expandKey(key, false)
    val buffer: Array[Int] = byte.grouped(4).map(setHolder).toArray
    decipher(buffer, workingKey, (key.length / 4) + 6)
    val result = ByteBuffer.allocate(16).order(ByteOrder.LITTLE_ENDIAN)
    buffer.foreach(result.putInt)
    result.array()
    //END_CHALLENGE
  ...
  //BEGIN_CHALLENGE
  def encipher(buffer: Array[Int], workingKey: Array[Array[Int]], rounds: Int): Unit = {
    var C0 = buffer(0)
    var C1 = buffer(1)
    var C2 = buffer(2)
    var C3 = buffer(3)
  ...
  def decipher(buffer: Array[Int], workingKey: Array[Array[Int]], rounds: Int): Unit = {
    var C0 = buffer(0)
    var C1 = buffer(1)
    var C2 = buffer(2)
    var C3 = buffer(3)
    ...
    for (i <- rounds until 2 by -2) {
      wk = workingKey(i - 1)
      r0 = InvTable0(C0 & 0xFF) ^ InvTable1((C3 >>> 8) & 0xFF) ^ InvTable2((C2 >>> 16) & 0xFF) ^ InvTable3(C1 >>> 24) ^ wk(0)
      r1 = InvTable0(C1 & 0xFF) ^ InvTable1((C0 >>> 8) & 0xFF) ^ InvTable2((C3 >>> 16) & 0xFF) ^ InvTable3(C2 >>> 24) ^ wk(1)
      r2 = InvTable0(C2 & 0xFF) ^ InvTable1((C1 >>> 8) & 0xFF) ^ InvTable2((C0 >>> 16) & 0xFF) ^ InvTable3(C3 >>> 24) ^ wk(2)
      r3 = InvTable0(C3 & 0xFF) ^ InvTable1((C2 >>> 8) & 0xFF) ^ InvTable2((C1 >>> 16) & 0xFF) ^ InvTable3(C0 >>> 24) ^ wk(3)
      wk = workingKey(i - 2)
      C0 = InvTable0(r0 & 0xFF) ^ InvTable1((r3 >>> 8) & 0xFF) ^ InvTable2((r2 >>> 16) & 0xFF) ^ InvTable3(r1 >>> 24) ^ wk(0)
      C1 = InvTable0(r1 & 0xFF) ^ InvTable1((r0 >>> 8) & 0xFF) ^ InvTable2((r3 >>> 16) & 0xFF) ^ InvTable3(r2 >>> 24) ^ wk(1)
      C2 = InvTable0(r2 & 0xFF) ^ InvTable1((r1 >>> 8) & 0xFF) ^ InvTable2((r0 >>> 16) & 0xFF) ^ InvTable3(r3 >>> 24) ^ wk(2)
      C3 = InvTable0(r3 & 0xFF) ^ InvTable1((r2 >>> 8) & 0xFF) ^ InvTable2((r1 >>> 16) & 0xFF) ^ InvTable3(r0 >>> 24) ^ wk(3)
    }
    ...
    //Final round of SubBytes, ShiftRows and AddRoundKey
    wk = workingKey(0)
    C0 = (InvSbox(r0 & 0xFF) & 0xFF) ^ ((InvSbox((r3 >>> 8) & 0xFF) & 0xFF) << 8) ^ ((InvSbox((r2 >>> 16) & 0xFF) & 0xFF) << 16) ^ ((InvSbox(r1 >>> 24) & 0xFF) << 24) ^ wk(0)
    C1 = (InvSbox(r1 & 0xFF) & 0xFF) ^ ((InvSbox((r0 >>> 8) & 0xFF) & 0xFF) << 8) ^ ((InvSbox((r3 >>> 16) & 0xFF) & 0xFF) << 16) ^ ((InvSbox(r2 >>> 24) & 0xFF) << 24) ^ wk(1)
    C2 = (InvSbox(r2 & 0xFF) & 0xFF) ^ ((InvSbox((r1 >>> 8) & 0xFF) & 0xFF) << 8) ^ ((InvSbox((r0 >>> 16) & 0xFF) & 0xFF) << 16) ^ ((InvSbox(r3 >>> 24) & 0xFF) << 24) ^ wk(2)
    C3 = (InvSbox(r3 & 0xFF) & 0xFF) ^ ((InvSbox((r2 >>> 8) & 0xFF) & 0xFF) << 8) ^ ((InvSbox((r1 >>> 16) & 0xFF) & 0xFF) << 16) ^ ((InvSbox(r0 >>> 24) & 0xFF) << 24) ^ wk(3)
  ...
  def setHolder(array: Array[Byte]): Int = ((array(3) & 0xFF) << 24) | ((array(2) & 0xFF) << 16) | ((array(1) & 0xFF) << 8) | (array(0) & 0xFF)

  def expandKey(key: Array[Byte], encrypt: Boolean = true): Array[Array[Int]] = {
    val keyLength: Int = key.length / 4
    val rounds = keyLength + 6
    ...
    if (!encrypt)
      for (i <- 1 until rounds)
        workingKey(i) = workingKey(i).map(InvMcol)

    workingKey
  }

  def InvMcol(x: Int): Int = {
    var t0 = x
  ...
  def FFmulX(x: Int): Int = ((x & m2) << 1) ^ (((x & m1) >>> 7) * m3)

  def FFmulX2(x: Int): Int = {
    var t1 = x & m4
    t1 ^= t1 >>> 1
    ((x & m5) << 2) ^ (t1 >>> 2) ^ (t1 >>> 5)
  }

  def ToInt(buffer: Array[Byte], offset: Int): Int = (buffer(offset) & 0xFF) | (buffer(offset + 1) & 0xFF) << 8 | (buffer(offset + 2) & 0xFF) << 16 | (buffer(offset + 3) & 0xFF) << 24

  def Shift(x: Int, shift: Int): Int = (x >>> shift) | (x << (32 - shift))

  def SubWord(x: Int): Int = Sbox(x & 0xFF) & 0xFF |
    (Sbox((x >>> 8) & 0xFF) & 0xFF) << 8 |
    (Sbox((x >>> 16) & 0xFF) & 0xFF) << 16 |
    (Sbox((x >>> 24) & 0xFF) & 0xFF) << 24

  final val m1 = 0x80808080
  final val m2 = 0x7f7f7f7f
  final val m3 = 0x0000001b
  final val m4 = 0xC0C0C0C0
  final val m5 = 0x3f3f3f3f

  lazy final val Sbox: Array[Byte] = Array(
  ...
  lazy final val InvSbox: Array[Byte] = Array(
  ...
  lazy final val Rcon = Array(
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
    0x2f, 0x5e, 0xbc, 0x63, 0xc6, 0x97, 0x35, 0x6a, 0xd4, 0xb3, 0x7d, 0xfa, 0xef, 0xc5, 0x91
  )

  final val Table0: Array[Int] = Array(
  ...
  final val Table1: Array[Int] = Array(
  ...
  final val Table2: Array[Int] = Array(
  ...
  final val Table3: Array[Int] = Array(
  ...
  lazy final val InvTable0 = Array(
  ...
  lazy final val InvTable1 = Array(
  ...
  lazy final val InvTable2 = Array(
  ...
  lazy final val InvTable3 = Array (
